...
if (isset($_GET['update_id'])) {
    ...
    $sql = "SELECT appointments.id, trainers.trainer_id, trainers.name AS trainer_name, appointments.appointment_date, appointments.appointment_time 
      ...
      WHERE id = '$update_id' AND user_id = '".
      $_SESSION['user_id']."'";
    ...
    if ($appointment) {
        ...
        if (isset($_POST['update'])) {
            ...
            $update_sql = "UPDATE appointments SET appointment_date ='$appointment_date',  appointment_time='$appointment_time', trainer_id='$trainer_id' WHERE id='$update_id' AND user_id='".$_SESSION['user_id']."'";

            if ($conn->query($update_sql) === TRUE) {
                echo "<script>alert('Appointment updated successfully!'); window.location.href = 'view_appointment.php';</script>";
            } else {
    ...
    echo "<script>alert('No appointment selected for update.'); window.location.href = 'view_appointment.php';</script>";
}
?>
